...
chiper_suites = https.filter(lambda row: row.p443.https.tls is not None and row.p443.https.tls.cipher_suite is not None)\
.map(lambda row: (row.p443.https.tls.cipher_suite.name, 1))\
